...
\paragraph{Cell model:} Cells populate a static unstructured lattice (max. 1 cell per lattice site). They can be either \emph{dividing}, \emph{quiescent} or \emph{necrotic}. If dividing, cells can perform a birth and death process. After a cell division it can either reenter the cell cycle or become quiescent. A cell in quiescence can perform a death process or reenter the cell cycle and perform a birth process. In the necrotic case, cells perform a lysis process. The respective transition rates are defined by the following equations:
\begin{align}
	...
	k_{div}  &= \sI{k_{div}^{max}} \cdot  f_{div}^{a}  \cdot f_{div}^{l}  \cdot  f_{div}^{w}  \cdot f_{div}^{o}  \cdot p_{div} \\
	k_{qui}  &= \sI{k_{div}^{max}} \cdot  f_{div}^{a} \cdot  f_{div}^{l} \cdot   f_{div}^{w}  \cdot f_{div}^{o}  \cdot  (1-p_{div}) \\
...
\begin{align}
	...
	f_{div}^{w} &= (1 - 0.5 \cdot H(w-\sII{w^{div}})) \cdot H(\sII{n^{max}} - n_{w,o}) , \\
	f_{div}^{o} &= (1 - 0.5 \cdot H(\sII{o^{div}} - o)) \cdot H(\sII{n^{max}} - n_{w,o}),
\end{align}
...
\end{align}
and $e^{div}$, $p_{ATP}^{div}$, $p_{ATP}^{nec}$, $l^{div}$, $l^{nec}$, $w^{div}$, $o^{div}$ and $n^{max}$ are the critical values.

The division rate further depends on the shortest distance a cell would have to push surrounding cells away in order to free adjacent space for cell growth and division:
\begin{align}
